...
def welcome_msg(name, points):
    return (
        ...
        "┣ ✏️ Or send Telegram ID manually\n"
        "┣ 🔴 1 Search = 1 Point\n"
...
#   SEARCH FUNCTION
# ═══════════════════════════════════════════════════
def do_search(cid, uid, target_id, db):
    if db[uid]['points'] < 1:
        send(cid,
            "💎〰️💎〰️💎〰️💎〰️💎〰️\n"
            "   😔 <b>𝗢𝗨𝗧 𝗢𝗙 𝗣𝗢𝗜𝗡𝗧𝗦!</b>\n"
    ...
    try:
        r      = requests.get(f"{SEARCH_API}?key={API_KEY}&userid={target_id}", timeout=15)
...
def handle_message(msg):
    ...
    # ── TARGET button — user_shared ──────────────────
    if 'user_shared' in msg:
        shared_uid = str(msg['user_shared']['user_id'])
        if not is_member(uid):
            ...
            save_db(db)
            return
        do_search(cid, uid, shared_uid, db)
        return
